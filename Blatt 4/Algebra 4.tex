...
\emph{Die kürzere Lösung der ganzen Aufgabe besteht darin, in $G_n$ den Kern des Gruppenhomomorphismus
\begin{center}
\begin{tikzpicture}[node distance = 2 cm]
 \node (G) {$G$};
 \node[right of = G] (Gab1) {$G/[G,G]$};
 \node[right of = Gab1] (Gab2) {$G/[G,G]$};
 \draw[->>] (G) to node[anchor=south] {can.} (Gab1);
 \draw[->] (Gab1) to (Gab2);
\end{tikzpicture}\\
\end{center}
mit $g \mapsto [g] \mapsto [g]^n = [g^n]$ zu erkennen. Das die zweite Abbildung ein Gruppenhomomorphismus ist, folgt daraus, dass $G/[G,G]$ abelsch ist.
}
